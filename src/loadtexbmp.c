...
/* only works on 1024x1024 slices */
static void getslice(unsigned char *image, int fromX, int fromY, unsigned char *slice)
{
  int y;
  ...
  for (y =  0; y < 1024; y++) {
    fromLine = image + (fromY+y)*4096*3 + (fromX*3);
    toLine = slice + (y*1024)*3;
    memcpy(toLine, fromLine, 1024*3);
    }
}
 ...
 *  Load texture from BMP file
 */
unsigned int LoadCubeBMP(const char* file)
{
   unsigned int   texture;    // Texture name
   FILE*          f;          // File pointer
   unsigned short magic;      // Image magic
   ...
   if ((dx != 4096) || (dy != 3072)) Fatal("%s needs to be 4096 x 3072\n",file);

   //  Allocate image memory
   size = 3*dx*dy;
   image = (unsigned char*) malloc(size);
   slice = (unsigned char *) malloc(1024*1024*3);
   if (!image) Fatal("Cannot allocate %d bytes of memory for image %s\n",size,file);
   //  Seek to and read image
   if (fseek(f,off,SEEK_SET) || fread(image,size,1,f)!=1) Fatal("Error reading data from image %s\n",file);
   fclose(f);
